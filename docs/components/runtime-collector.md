...
| `existing_certificates` | empty | None available. With `certificates.verification: true` (default), **rejected at install time**, since the certificate can be neither verified nor accepted unverified. With `certificates.verification: false`, verification is skipped instead. |
