...
| `existing_certificates` | empty | None available. With `certificates.verification: false`, verification is skipped. With `certificates.verification: true` (default), the certificate cannot be verified and the connection fails at runtime — set `existing_ca_secret_name`, use `generate_self_signed_certificates`, or disable verification. |
