...
| [`max_connections`](https://dev.mysql.com/doc/refman/8.0/en/server-system-variables.html#sysvar_max_connections)                                       | `70 * (# of backends) + 10 * (# of Keycloak) + 10 (cron service)` | Must be set according to the number of backend services and Keycloak instances. |
